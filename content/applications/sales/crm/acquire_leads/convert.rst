...
To activate the *leads* setting, navigate to :menuselection:`CRM app --> Configuration -->
Settings`, and check the box labeled, :guilabel:`Leads`. Then, click :guilabel:`Save`.
...
.. note::
   If a lead, or an opportunity, already exists in the database with this customer, Odoo
   automatically suggests to merge with that lead/opportunity. For more information on merging leads
...
contact names. If a similar lead/opportunity is found, a :guilabel:`Similar Lead` smart button is
added to the top of the lead/opportunity record.
...
button. This opens a kanban view with only the similar leads/opportunities. Click into each card
to view the details for each lead/opportunity, and confirm if they should be merged.
...
Once it's confirmed the leads/opportunities should be merged, return to the kanban view using
breadcrumbs, or by clicking the smart button. Click the :guilabel:`☰ (three vertical lines)` icon
...
Below those fields, the :guilabel:`Select Leads/Opportunities` to merge, along with their related
...
.. note::
   ...
   the chatter, and the information fields, for easy access.
